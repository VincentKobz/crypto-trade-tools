...
def launch_backtracing(test_all, pair_name, date, inter, strategy, usd, taker_fee, maker_fee):
    ...
    bot = Bot(float(usd), 0, strategy, df_test, taker_fee, maker_fee, strat_array)
    ...
    if test_all == 'false':
        back_trace = main.BackTrace(bot)
        res.append(back_trace.result())
        print('One strategy')
        return res
